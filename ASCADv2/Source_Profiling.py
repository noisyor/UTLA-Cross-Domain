...
X_attack_source = np.array(source_file['Attack_traces/traces'], dtype=np.int8)[:source_test_num,5000:5000+1400]
Y_attack_source = np.array(source_file['Attack_traces/labels'])[:train_num]
...
real_key_profile = source_file['Profiling_traces/metadata']['key'][np.arange(train_num), Y_train_source_order]
plaintexts_source_train = source_file['Profiling_traces/metadata']['plaintext'][np.arange(train_num), Y_train_source_order]

#breakpoint()
# to load ciphertexts

real_key_attack = source_file['Attack_traces/metadata']['key'][np.arange(source_test_num), Y_attack_source_order]
plaintexts_source_attack = source_file['Attack_traces/metadata']['plaintext'][np.arange(source_test_num), Y_attack_source_order]
...
class UTLA_Net(nn.Module):
    ...
    def forward(self, input):
        ...
        x = self.features_4(x)
        x = x.view(x.size(0), -1)
        source = self.classifier_1(x)
        source = self.classifier_2(source)
        source = self.classifier_3(source)
