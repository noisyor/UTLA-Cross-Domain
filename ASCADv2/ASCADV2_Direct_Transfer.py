...
def multilabel_to_categorical(Y):
    y = {}
    y['alpha_output'] = F.one_hot(torch.tensor(Y['alpha_mask'], dtype=torch.int64), num_classes=256).float()
    y['beta_output'] = F.one_hot(torch.tensor(Y['beta_mask'], dtype=torch.int64), num_classes=256).float()
    for i in range(16):
        y['sbox_' + str(i) + '_output'] = F.one_hot(torch.tensor(Y['sbox_masked'][:, i], dtype=torch.int64), num_classes=256).float()
    for i in range(16):
        y['permind_' + str(i) + '_output'] = F.one_hot(torch.tensor(Y['perm_index'][:, i], dtype=torch.int64), num_classes=16).float()
    return y
...
def remove_operation(before, operand, operation, batch_size=1000):
    """
    Safely computes inverse operation: Pr(before | t) from Pr(operation(before, operand) | t)
    using batching to avoid high memory usage. Works with NumPy arrays or PyTorch tensors.

    Args:
        before: shape [N, 256] — e.g., Sbox-masked output probs
        operand: shape [N, 256] — e.g., one-hot alpha or beta distribution
        operation: GF(256) operation (e.g., addGF256 or mulGF256)
        batch_size: batch size to control memory

    Returns:
        after: same type and shape as input, [N, 256]
    """
    # Check if input is torch
    is_torch = isinstance(before, torch.Tensor)

    # Convert to numpy if needed
    if is_torch:
        before_np = before.detach().cpu().numpy()
        operand_np = operand.detach().cpu().numpy()
    else:
        before_np = before
        operand_np = operand

    N = before_np.shape[0]
    after_np = np.zeros((N, 256), dtype=np.float32)

    for batch_start in range(0, N, batch_size):
        batch_end = min(batch_start + batch_size, N)
        before_batch = before_np[batch_start:batch_end]  # [B, 256]
        operand_batch = operand_np[batch_start:batch_end]  # [B, 256]

        for j in range(256):
            indices = [operation(i, j) for i in range(256)]
            rearranged = before_batch[:, indices]  # [B, 256]
            after_np[batch_start:batch_end, j] = np.sum(rearranged * operand_batch, axis=1)

    # Convert back to torch if needed
    if is_torch:
        after = torch.tensor(after_np, dtype=before.dtype, device=before.device)
    else:
        after = after_np
