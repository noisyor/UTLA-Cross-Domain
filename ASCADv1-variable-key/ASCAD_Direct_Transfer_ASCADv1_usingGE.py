...
def GE_diff_loss(source_preds, plaintext_batch, real_key_batch, trace_num_max=2000):
    """
...
real_key_profile = target_file['Profiling_traces/metadata']['key'][:,2]
real_key_attack = target_file['Attack_traces/metadata']['key'][:,2]
...
X_train_source = np.array(target_file['Profiling_traces/traces'], dtype=np.int8)
X_attack_source = np.array(target_file['Attack_traces/traces'], dtype=np.int8)
Y_train_source = np.array(target_file['Profiling_traces/labels'])
Y_attack_source = np.array(target_file['Attack_traces/labels'])

# to load ciphertexts
plaintexts_source_attack = target_file['Attack_traces/metadata']['plaintext'][:,2]
plaintexts_source_train = target_file['Profiling_traces/metadata']['plaintext'][:,2]
