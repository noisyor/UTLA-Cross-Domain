...
def encoder_rep(epoch, model, profile_model):
    ...
    plt.show()
    plt.savefig('./figures/PCA_UTLA_crossDev_{}_to_{}_'.format(source_device_id, target_device_id) + '_epoch_{}'.format(epoch)+'.png') 
...
def plot_guessing_entropy(preds, real_key, device_id, model_flag):
    ...
    plt.tight_layout()  # Automatically adjust padding to prevent clipping  
    plt.show()
    plt.savefig('./figures/entropy_UTLA_crossDev_{}_to_{}_'.format(source_device_id, device_id) + model_flag + '.png') 
    np.save('./results/entropy_UTLA_crossDev_{}_to_{}_'.format(source_device_id, device_id) + model_flag, guessing_entropy)
...
X_train_source = np.array(source_file['Profiling_traces/traces'], dtype=np.int8)
X_attack_source = np.array(source_file['Attack_traces/traces'], dtype=np.int8)
Y_train_source = np.array(source_file['Profiling_traces/labels'])
Y_attack_source = np.array(source_file['Attack_traces/labels'])
...
# to load ciphertexts
plaintexts_source_attack = source_file['Attack_traces/metadata']['plaintext'][:,2]
plaintexts_source_train = source_file['Profiling_traces/metadata']['plaintext'][:,2]
...
if(set_UTLA_train==1):
    ...
    #checkpoint_target = torch.load('./models/UTLA-final_XMEGA_EM_device{}_to_{}.pth'.format(source_device_id, target_device_id),weights_only=True)
    ...
    for epoch in range(1, finetune_epoch + 1):
        ...
        plt.tight_layout()  # Automatically adjust padding to prevent clipping    
        plt.show()
        plt.savefig('./figures/GE_UTLA_XMEGA_EM_{}_to_{}_'.format(source_device_id, target_device_id) + '_training' + '.png') 
        np.save('./results/GE_UTLA_XMEGA_EM_{}_to_{}_'.format(source_device_id, target_device_id) + '_training', Intermediate_GE)
        ...
        plt.tight_layout()  # Automatically adjust padding to prevent clipping  
        plt.show()
        plt.savefig('./figures/NTGE_UTLA_XMEGA_EM_{}_{}_'.format(source_device_id, target_device_id) + '_training' + '.png') 
        np.save('./results/NTGE_UTLA_XMEGA_EM_{}_{}_'.format(source_device_id, target_device_id) + '_training', Intermediate_NTGE)    
    ...
    torch.save({
        ...
        }, './models/UTLA-final_XMEGA_EM_device{}_to_{}.pth'.format(source_device_id, target_device_id))
...
checkpoint = torch.load('./models/UTLA-final_XMEGA_EM_device{}_to_{}.pth'.format(source_device_id, target_device_id),weights_only=True)
