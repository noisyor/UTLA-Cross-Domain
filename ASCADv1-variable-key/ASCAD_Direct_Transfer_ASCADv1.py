...
def plot_guessing_entropy(preds, real_key, device_id, model_flag):
    ...
    plt.ylim((0, 128))
    plt.show()
    plt.savefig('./figures/entropy_ASCADv1_{}_to_{}_'.format(source_device_id, device_id) + model_flag + '.png') 
    np.save('./results/entropy_ASCADv1_{}_to_{}_'.format(source_device_id, device_id) + model_flag, guessing_entropy)
...
train_num = env_int("UTLA_TRAIN_NUM", 100000)
...
class_num = 256
trace_num_max = env_int("UTLA_TRACE_NUM_MAX", 5000)
...
kwargs_source_test = {
        'trs_file': X_attack_source[0:source_test_num,:],
        'label_file': Y_attack_source[0:source_test_num],
...
if(train_first_time==1):
    ...
    for epoch in range(1, total_epoch + 1):
        ...
        if(print_intermediate_GE == 1):
            ...
            plt.show()
            plt.savefig('./figures/GE_ASCADv1_{}_'.format(source_device_id) + '_training' + '.png') 
            np.save('./results/GE_ASCADv1_{}_'.format(source_device_id) + '_training', Intermediate_GE)
            
    torch.save({
        'epoch': epoch,
        'model_state_dict': Profile_model.state_dict(),
        'optimizer_state_dict': optimizer.state_dict()
        }, './models/pre-trained_device_ASCADv1{}.pth'.format(source_device_id))
    ...
    if(print_intermediate_GE == 1):    
        ...
        plt.tight_layout()  # Automatically adjust padding to prevent clipping 
        plt.show()
        plt.savefig('./figures/NTGE_ASCADv1_{}_'.format(source_device_id) + '_training' + '.png') 
        np.save('./results/NTGE_ASCADv1_{}_'.format(source_device_id) + '_training', Intermediate_NTGE)
    ...
    checkpoint = torch.load('./models/pre-trained_device_ASCADv1{}.pth'.format(source_device_id),weights_only=True)
