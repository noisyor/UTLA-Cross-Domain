...
source_device= int(sys.argv[1]) 
target_device= int(sys.argv[2]) 
...
def plot_guessing_entropy(preds, real_key, device_id, model_flag):
    ...
    plt.ylim((0, 128))
    plt.show()
    plt.savefig('./figures/entropy_'+ labeling_method + '_{}_to_{}_'.format(source_device_id, device_id) + model_flag + '.png') 
...
model_path = './models/pre-trained_device{}.pth'.format(source_device_id)
...
try:
    ...
    checkpoint = None  # or some default behavior
...
with torch.no_grad():
    ...
    test(Profile_model, target_device_id, model_flag='direct_transfer_EM_to_Power')
