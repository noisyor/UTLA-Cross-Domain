...
def test(model, profile_model, device_id, model_flag='pretrained'):
    ...
    for i in range(0,iter_num):
        ...
        target_preds = softmax(target_preds)
        target_preds = compute_Pr2(target_preds) # compute Pr2 for the output of the model
        test_preds_all[i*batch_size:(i+1)*batch_size, :] = target_preds
...
def encoder_rep(epoch, model, profile_model):
    ...
    profile_model.eval()
    test_num = target_test_num
    target_feat_preds = torch.zeros((test_num, 64), dtype=torch.float, device='cpu')
    source_feat_preds = torch.zeros((test_num, 64), dtype=torch.float, device='cpu')
    iter_target = iter(target_test_loader)
    iter_source = iter(source_test_loader)
    iter_num = len(target_test_loader)
    for i in range(0,iter_num):
...
def test_intermediate(model):
    ...
    for i in range(0,trace_num_max):
        ...
        truelabel = int(Sbox[trueState]// 16 == 0)
        #breakpoint()
        for key_guess in range(0, 256):
            initialState = plaintext[int(ordering[i])] ^ key_guess
            label = int(Sbox[initialState]// 16 == 0)
...
def plot_guessing_entropy(preds, real_key, device_id, model_flag):
    ...
    for i in range(0,trace_num_max):
        for key_guess in range(0, 256):
            initialState = int(plaintext[i]) ^ key_guess
            label = int(Sbox[initialState]// 16 == 0)
            trueState = int(plaintext[i]) ^ real_key
            truelabel = int(Sbox[trueState]// 16 == 0)
    ...
    plt.tight_layout()  # Automatically adjust padding to prevent clipping  
    plt.show()
    plt.savefig('./figures/GE_UTLA_crossdevXMEGA_{}_to_{}_'.format(source_device_id, device_id) + model_flag + '.png') 
    np.save('./results/GE_UTLA_crossdevXMEGA_{}_to_{}_'.format(source_device_id, device_id) + model_flag, guessing_entropy)
...
target_file_path = '../XMEGA-Power/Data/device0'+str(target_device)+'/'
...
class UTLA_Net(nn.Module):
    ...
    def forward(self, target):
        ...
        #target data flow
        if(target.shape[2] == 500): # for compatibility with the source domain    
            x = target[:, :, -250:]
        else:
            x = target
        x = self.features_1(x)
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.decoder = nn.Sequential(
            nn.Unflatten(1, (64, 1)),                     # [B, 64] → [B, 64, 1]
            nn.ConvTranspose1d(64, 32, kernel_size=2, stride=2),  # → [B, 32, 2]
            nn.SELU(),
            nn.BatchNorm1d(32),

            nn.ConvTranspose1d(32, 16, kernel_size=3, stride=3),  # → [B, 16, 6]
            nn.SELU(),
            nn.BatchNorm1d(16),

            nn.ConvTranspose1d(16, 8, kernel_size=9, stride=9),   # → [B, 8, 54]
            nn.SELU(),
            nn.BatchNorm1d(8),

            nn.ConvTranspose1d(8, 1, kernel_size=5, stride=5),    # → [B, 1, 270]
        )

    def forward(self, x):
        x = self.decoder(x)
        return x[:, :, -250:]  # crop or slice to exact output length
...
if(auto_enc_train==1):
    ...
    for epoch in range(10):
        ...
        for step in range(0,num_iter):
            ...
            loss = criterion(output, target_data[:,:,-250:])
            loss.backward()
            optimizer.step()
            if step % log_interval == 0:
    ...
    torch.save({
        ...
        },'./models/pretrained_encoder{}_to_{}.pth'.format(source_device_id, target_device_id))
...
if(set_UTLA_train==1):
    ...
    # checkpoint_target = torch.load('./models/pre-trained_XMEGA-Power-CL-device{}.pth'.format(5),weights_only=True)
    ...
    enc_ckpt = torch.load('./models/pretrained_encoder{}_to_{}.pth'.format(source_device_id, target_device_id))
    ...
    for epoch in range(1, finetune_epoch + 1):
        ...
        if(epoch>1 and epoch<=5):
            optimizer_critic = optim.SGD([
                {'params': discriminator.discriminator.parameters()},
                 ],lr=d_learning_rate/100, weight_decay=0.0005, momentum=0.9)
            optimizer_model = optim.SGD([
                {'params': UTLA_training_model.features_1.parameters()},
                {'params': UTLA_training_model.features_2.parameters()},
                {'params': UTLA_training_model.features_3.parameters()},
                {'params': UTLA_training_model.features_4.parameters()}
            ],lr=c_learning_rate/100, weight_decay=0.0005, momentum=0.9)
        elif(epoch>5):
            optimizer_critic = optim.SGD([
                {'params': discriminator.discriminator.parameters()},
                 ],lr=d_learning_rate/1000, weight_decay=0.0005, momentum=0.9)
            optimizer_model = optim.SGD([
                {'params': UTLA_training_model.features_1.parameters()},
                {'params': UTLA_training_model.features_2.parameters()},
                {'params': UTLA_training_model.features_3.parameters()},
                {'params': UTLA_training_model.features_4.parameters()}
            ],lr=c_learning_rate/1000, weight_decay=0.0005, momentum=0.9)
        ...
        plt.tight_layout()  # Automatically adjust padding to prevent clipping  
        plt.show()
        plt.savefig('./figures/GE_UTLA_crossdevXMEGA_{}_to_{}_'.format(source_device_id, target_device_id) + '_training' + '.png') 
        np.save('./results/GE_UTLA_crossdevXMEGA_{}_to_{}_'.format(source_device_id, target_device_id) + '_training', Intermediate_GE)
        UTLA_train(epoch, UTLA_training_model, UTLA_profile_model, discriminator)
    torch.save({
        'epoch': epoch,
        'model_state_dict': UTLA_training_model.state_dict(),
        }, './models/UTLA-final_crossdevXMEGA_device{}_to_{}.pth'.format(source_device_id, target_device_id))
    ...
    plt.tight_layout()  # Automatically adjust padding to prevent clipping   
    plt.show()
    plt.savefig('./figures/NTGE_UTLA_crossdevXMEGA_{}_{}_'.format(source_device_id, target_device_id) + '_training' + '.png') 
    np.save('./results/NTGE_UTLA_crossdevXMEGA_{}_{}_'.format(source_device_id, target_device_id) + '_training', Intermediate_NTGE)    
...
checkpoint = torch.load('./models/UTLA-final_crossdevXMEGA_device{}_to_{}.pth'.format(source_device_id, target_device_id),weights_only=True)
